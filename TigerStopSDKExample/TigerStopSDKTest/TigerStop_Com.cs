...
namespace TigerStopSDKExample
{
    public class TigerStop_Com
    {
        ...
        //  -  SERIAL CONSTRUCTOR
        public TigerStop_Com(int baud, string comPort)
        {
            this.baudrate = baud;
            ...
            bkgndCycle.WorkerReportsProgress = false;
            bkgndCycle.WorkerSupportsCancellation = true;
            bkgndCycle.DoWork += new DoWorkEventHandler(bkgndCycle_DoWork);
            bkgndCycle.RunWorkerCompleted += new RunWorkerCompletedEventHandler(bkgndCycle_Completed);
        }

        //  =  =  =  BACKGROUND WORKERS  =  =  =
        //
        // --- private void bkgndCycle_DoWork(object sender, DoWorkEventArgs e) ---
        /// <summary>
        /// This background worker is only fired once we've seen a 'MTS' from the machine, informing us that we've starting a move tool cycle.
        /// While we trust that the machine is actually cycling, its best to double check that its actually doing that by looking first for the 'DMS' ack,
        /// signaling that the dead man switch is off. Then looking for the 'DMF' ack, signaling that the dead man switch is back on.
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void bkgndCycle_DoWork(object sender, DoWorkEventArgs e)
        {
            if (lastAck.Acknowledgement.Contains("DMS"))
            {
                isDmOff = true;
            }
            
            if (lastAck.Acknowledgement.Contains("DMF") && isDmOff)
            {
                isDmOn = true;
            }
        }

        // --- private void bkgndCycle_Completed(object sender, RunWorkerCompletedEventArgs e) ---
        /// <summary>
        /// This RunWorkerCompleted event handler just checks to see if the machine has signaled to us that the dead man switch has gone off and come back on,
        /// signaling the completion of an entire tool cycle. If we have not seen both signals, then we have to assume that we're still in the middle of a cycle.
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void bkgndCycle_Completed(object sender, RunWorkerCompletedEventArgs e)
        ...
        private void HandleAck()
        {
            ...
            {
                // Clear the NACK from the buffer.
                readBuffer.Clear();
        ...
        // --- private void ClearCommand(bool allCommands) ---
        /// <summary>
        ...
        // --- public void ChangeSetting(string command, int index) ---
        /// <summary>
        ...
        // --- public void ClearWriteBuffer(bool all) ---
        /// <summary>
        /// Some acks from the machine don't always make it back to the system, so 'writeBuffer' needs to be cleared so that other commands can be sent down.
        /// </summary>
        /// <param name="all"> A 'bool' that denotes whether to clear out all of the commands in 'writeBuffer', or just the first. </param>
        protected void ClearWriteBuffer(bool all)
        {
            ClearCommand(all);
        }
